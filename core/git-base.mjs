...
export async function githubCreatePullRequest(cwd,{remote='origin',base='main',head=null,title='',body='',draft=false,expectedHeadSha=null,token=null,env=process.env,fetchImpl=globalThis.fetch}={}){
  ...
  const expectedRaw=String(expectedHeadSha||'').trim();if(!expectedRaw)return {ok:false,prPerformed:false,externalSideEffectPerformed:false,error:'EXPECTED_HEAD_SHA_REQUIRED',repository:ctx.repository,expectedHeadSha:null};
  const expected=resolveExpectedCommit(cwd,expectedRaw);if(!expected.ok)return {ok:false,prPerformed:false,externalSideEffectPerformed:false,error:expected.error,repository:ctx.repository,expectedHeadSha:expectedRaw||null};
  const headers={accept:'application/vnd.github+json',authorization:`Bearer ${ctx.token}`,'content-type':'application/json','x-github-api-version':'2022-11-28'};
  const preflight=await apiJson(fetchImpl,`https://api.github.com/repos/${ctx.repository}/git/ref/heads/${encodeURIComponent(selectedHead)}`,{method:'GET',headers});
  const remoteHeadSha=String(preflight.data?.object?.sha||'').trim();
  if(!preflight.ok)return {ok:false,prPerformed:false,externalSideEffectPerformed:false,repository:ctx.repository,expectedHeadSha:expected.sha,remoteHeadSha:null,status:preflight.status,error:'PR_CREATE_HEAD_PREFLIGHT_FAILED',detail:preflight.error};
  if(!/^[0-9a-f]{40,64}$/i.test(remoteHeadSha))return {ok:false,prPerformed:false,externalSideEffectPerformed:false,repository:ctx.repository,expectedHeadSha:expected.sha,remoteHeadSha:remoteHeadSha||null,status:preflight.status,error:'PR_CREATE_HEAD_PREFLIGHT_RECEIPT_INVALID'};
  if(remoteHeadSha.toLowerCase()!==expected.sha.toLowerCase())return {ok:false,prPerformed:false,externalSideEffectPerformed:false,repository:ctx.repository,expectedHeadSha:expected.sha,remoteHeadSha,status:preflight.status,error:'PR_CREATE_HEAD_DRIFT'};
  const result=await apiJson(fetchImpl,`https://api.github.com/repos/${ctx.repository}/pulls`,{method:'POST',headers,body:JSON.stringify({title:selectedTitle,head:selectedHead,base:String(base||'main'),body:String(body||''),draft:draft===true})});
  const prNumber=Number(result.data?.number);const returnedHeadSha=String(result.data?.head?.sha||'').trim();const headMatches=returnedHeadSha&&returnedHeadSha.toLowerCase()===expected.sha.toLowerCase();const receiptValid=result.ok&&Number.isInteger(prNumber)&&prNumber>0&&headMatches;const error=!result.ok?result.error:!Number.isInteger(prNumber)||prNumber<1?'PR_CREATE_RECEIPT_INVALID':!headMatches?'PR_CREATE_HEAD_MISMATCH':null;
  return {ok:receiptValid,prPerformed:result.ok,externalSideEffectPerformed:result.ok,repository:ctx.repository,number:Number.isInteger(prNumber)&&prNumber>0?prNumber:null,expectedHeadSha:expected.sha,remoteHeadSha,headSha:returnedHeadSha||null,url:result.data?.html_url||null,state:result.data?.state||null,status:result.status,error};
}
...
export async function gitRemoteDelivery(cwd,{explicitApproval=false,push=true,createPullRequest=false,merge=false,remote='origin',branch=null,base='main',title='',body='',draft=false,prNumber=null,mergeMethod='squash',expectedHeadSha=null,token=null,env=process.env,fetchImpl=globalThis.fetch}={}){
  ...
  if((push||createPullRequest||merge)&&!String(expectedHeadSha||'').trim())return {...result,error:'EXPECTED_HEAD_SHA_REQUIRED'};
