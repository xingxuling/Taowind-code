...
export class RunStore{
  ...
  create({goal,mode='WHOLE_ARTIFACT',dwac=null,tasks=[]}={}){
    const run={id:id(),protocol:'taowind-code.north-star-run.v0.2',goal:String(goal||'').trim(),status:'OBSERVING',mode,cycle:1,createdAt:now(),updatedAt:now(),dwac,tasks,evidence:[],events:[],changesets:[],validation:null,delivery:null,blocker:null,message:null};
  ...
  get(runId){const file=this.file(runId);if(!fs.existsSync(file))throw Object.assign(new Error('RUN_NOT_FOUND'),{code:'RUN_NOT_FOUND'});const run=JSON.parse(fs.readFileSync(file,'utf8'));if(run?.id!==runId)throw new Error('RUN_ID_MISMATCH');return run}
