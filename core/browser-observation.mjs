...
function normalizeCheck(raw={}){return {url:validateWebUrl(String(raw.url||'')),requiredSelectors:[...new Set((raw.requiredSelectors||raw.selectors||[]).map(String).filter(Boolean))].slice(0,48),requiredText:[...new Set((raw.requiredText||[]).map(String).filter(Boolean))].slice(0,32),titleIncludes:String(raw.titleIncludes||''),urlIncludes:String(raw.urlIncludes||''),forbidConsoleErrors:raw.forbidConsoleErrors!==false,forbidPageExceptions:raw.forbidPageExceptions!==false,forbidCriticalNetworkErrors:raw.forbidCriticalNetworkErrors!==false,screenshot:raw.screenshot!==false,settleMs:Math.max(100,Math.min(5000,Number(raw.settleMs||700)))}}
