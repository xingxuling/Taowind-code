...
export function repositorySummary(manifest,graph=null){
  const byExt={};for(const f of manifest)byExt[f.ext||'<none>']=(byExt[f.ext||'<none>']||0)+1;return {fileCount:manifest.length,extensions:Object.entries(byExt).sort((a,b)=>b[1]-a[1]).slice(0,16),topLevel:[...new Set(manifest.map(x=>x.path.split('/')[0]))].slice(0,80),semanticGraph:graph?{fingerprint:graph.fingerprint,...graph.summary}:null};
}
