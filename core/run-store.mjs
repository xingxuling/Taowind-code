...
const EVIDENCE_ID_RE=/^ev-[a-f0-9]{12}$/;
...
export class RunStore{
  ...
  create({goal,mode='WHOLE_ARTIFACT',dwac=null,tasks=[]}={}){
    ...
    if(!run.goal)throw new Error('GOAL_REQUIRED');if(!MODES.has(run.mode))throw new Error('INVALID_RUN_MODE');if(!validOptionalObject(run.dwac))throw new Error('INVALID_RUN_OPTIONAL_OBJECT');this.eventObject(run,'RUN_CREATED',{mode});atomicJson(this.file(run.id),run);return run;
  }
  get(runId){const file=this.file(runId);if(!fs.existsSync(file))throw Object.assign(new Error('RUN_NOT_FOUND'),{code:'RUN_NOT_FOUND'});const run=JSON.parse(fs.readFileSync(file,'utf8'));if(run?.id!==runId)throw new Error('RUN_ID_MISMATCH');if(run?.protocol!==RUN_PROTOCOL)throw new Error('UNSUPPORTED_RUN_PROTOCOL');if(!MODES.has(run?.mode))throw new Error('INVALID_RUN_MODE');if(!VALID.has(run?.status))throw new Error('INVALID_RUN_STATUS');if(typeof run?.goal!=='string'||!run.goal.trim())throw new Error('INVALID_RUN_GOAL');if(!Number.isInteger(run?.cycle)||run.cycle<1)throw new Error('INVALID_RUN_CYCLE');if(!validDateTime(run?.createdAt)||!validDateTime(run?.updatedAt))throw new Error('INVALID_RUN_TIMESTAMP');if(!validEventLedger(run?.events)||!validEvidenceLedger(run?.evidence)||!validChangesetLedger(run?.changesets))throw new Error('INVALID_RUN_LEDGER');if(run.events.length<1||run.events.at(-1).at!==run.updatedAt)throw new Error('INVALID_RUN_EVENT_HEAD');for(const field of ['dwac','validation','delivery'])if(!validOptionalObject(run?.[field]))throw new Error('INVALID_RUN_OPTIONAL_OBJECT');return run}
  ...
  eventObject(run,type,data={}){if(typeof type!=='string'||!type.trim())throw new Error('INVALID_RUN_EVENT_LEDGER');const event={seq:run.events.length+1,at:now(),type,data};run.events.push(event);run.updatedAt=event.at;return event}
  update(runId,patch={},eventType='RUN_UPDATED',eventData={}){const run=this.get(runId);if(Object.prototype.hasOwnProperty.call(patch,'id')&&patch.id!==run.id)throw new Error('RUN_ID_IMMUTABLE');if(Object.prototype.hasOwnProperty.call(patch,'protocol')&&patch.protocol!==RUN_PROTOCOL)throw new Error('RUN_PROTOCOL_IMMUTABLE');if(Object.prototype.hasOwnProperty.call(patch,'createdAt')&&!validDateTime(patch.createdAt))throw new Error('INVALID_RUN_TIMESTAMP');if(TERMINAL.has(run.status)&&Object.prototype.hasOwnProperty.call(patch,'status')&&patch.status!==run.status)throw new Error('RUN_TERMINAL');if(Object.prototype.hasOwnProperty.call(patch,'status')&&!VALID.has(patch.status))throw new Error('INVALID_RUN_STATUS');if(Object.prototype.hasOwnProperty.call(patch,'mode')&&!MODES.has(patch.mode))throw new Error('INVALID_RUN_MODE');if(Object.prototype.hasOwnProperty.call(patch,'goal')&&(typeof patch.goal!=='string'||!patch.goal.trim()))throw new Error('INVALID_RUN_GOAL');if(Object.prototype.hasOwnProperty.call(patch,'cycle')&&(!Number.isInteger(patch.cycle)||patch.cycle<1))throw new Error('INVALID_RUN_CYCLE');if(Object.prototype.hasOwnProperty.call(patch,'events')&&!validEventLedger(patch.events))throw new Error('INVALID_RUN_LEDGER');if(Object.prototype.hasOwnProperty.call(patch,'evidence')&&!validEvidenceLedger(patch.evidence))throw new Error('INVALID_RUN_LEDGER');if(Object.prototype.hasOwnProperty.call(patch,'changesets')&&!validChangesetLedger(patch.changesets))throw new Error('INVALID_RUN_LEDGER');for(const field of ['dwac','validation','delivery'])if(Object.prototype.hasOwnProperty.call(patch,field)&&!validOptionalObject(patch[field]))throw new Error('INVALID_RUN_OPTIONAL_OBJECT');Object.assign(run,patch);this.eventObject(run,eventType,eventData);atomicJson(this.file(runId),run);return run}
