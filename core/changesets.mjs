...
export class ChangesetStore{
 ...
 stage(runId,changes,{source='agent'}={}){
   ...
   for(const raw of changes){const op=raw?.op||'write';if(!['write','delete'].includes(op))throw new Error('UNSUPPORTED_CHANGE_OPERATION');const rel=path.posix.normalize(String(raw.path||'').replaceAll('\\','/'));if(!rel||rel==='.'||rel==='..'||rel.startsWith('../')||rel.startsWith('/')||/^[A-Za-z]:/.test(rel))throw new Error('INVALID_CHANGE_PATH');if(seenPaths.has(rel))throw new Error(`DUPLICATE_CHANGE_PATH:${rel}`);seenPaths.add(rel);const ancestor=this.workspace.ancestorState(rel);if(!ancestor.ok)throw new Error(`UNSUPPORTED_CHANGE_ANCESTOR:${rel}:${ancestor.ancestor}`);const before=this.workspace.stat(rel);if(before.exists&&before.type!=='file')throw new Error(`UNSUPPORTED_CHANGE_TARGET:${rel}`);let beforeBytes=null;if(before.exists)beforeBytes=this.workspace.readBytes(rel,MAX_FILE_BYTES);let after=null;if(op==='write'){after=Buffer.from(String(raw.content??''),'utf8');if(after.length>MAX_FILE_BYTES)throw new Error('CHANGE_FILE_TOO_LARGE');total+=after.length;if(total>MAX_TOTAL_BYTES)throw new Error('CHANGESET_TOO_LARGE')}
     if(raw.expectedSha256!==undefined&&raw.expectedSha256!==before.sha256)throw new Error(`PREIMAGE_MISMATCH:${rel}`);
     normalized.push({op,path:rel,before:{exists:before.exists,type:before.type||null,sha256:before.sha256,size:before.size,identity:before.identity||null,contentBase64:encode(beforeBytes)},after:{sha256:after?sha256Buffer(after):null,size:after?.length||0,contentBase64:encode(after)}})
   }
 ...
 apply(id){const doc=this.get(id);if(doc.status!=='STAGED')throw new Error('CHANGESET_NOT_STAGED');const preflight=[];
   for(const c of doc.changes){const ancestor=this.workspace.ancestorState(c.path);if(!ancestor.ok)throw new Error(`APPLY_CONFLICT:${c.path}`);const current=this.workspace.stat(c.path);const identityChanged=c.before.exists&&c.before.identity&&current.identity!==c.before.identity;if(current.exists!==c.before.exists||current.sha256!==c.before.sha256||identityChanged)throw new Error(`APPLY_CONFLICT:${c.path}`);preflight.push({change:c,current})}
   const applied=[];for(const {change:c} of preflight){if(c.op==='delete')this.workspace.remove(c.path);else this.workspace.write(c.path,decode(c.after.contentBase64).toString('utf8'));const post=this.workspace.stat(c.path);applied.push({path:c.path,op:c.op,sha256:post.sha256,exists:post.exists,identity:post.identity||null})}
 ...
 rollback(id){const doc=this.get(id);if(doc.status!=='APPLIED')throw new Error('CHANGESET_NOT_APPLIED');const preflight=[],appliedByPath=new Map((doc.applyReceipt?.files||[]).map(x=>[x.path,x]));
   for(const c of [...doc.changes].reverse()){const ancestor=this.workspace.ancestorState(c.path);if(!ancestor.ok)throw new Error(`ROLLBACK_CONFLICT:${c.path}`);const current=this.workspace.stat(c.path);const expectedPost=c.op==='delete'?null:c.after.sha256;const expectedIdentity=appliedByPath.get(c.path)?.identity||null;if(current.sha256!==expectedPost||current.exists!==(c.op!=='delete')||(expectedIdentity&&current.identity!==expectedIdentity))throw new Error(`ROLLBACK_CONFLICT:${c.path}`);preflight.push(c)}
   const restored=[];for(const c of preflight){if(!c.before.exists)this.workspace.remove(c.path);else{if(c.before.type!=='file')throw new Error(`ROLLBACK_UNSUPPORTED_TYPE:${c.path}`);this.workspace.write(c.path,decode(c.before.contentBase64).toString('utf8'))}const post=this.workspace.stat(c.path);restored.push({path:c.path,exists:post.exists,sha256:post.sha256,identity:post.identity||null})}
