...
test('_record persists null or lowercase 64-hex digest values',()=>withGate((gate,root)=>{const policyDigest='a'.repeat(64);const materializedDigest='b'.repeat(64);gate._record(receipt({policyDigest,materializedDigest}));const durable=JSON.parse(fs.readFileSync(path.join(root,'authority-receipts','auth-test.json'),'utf8'));assert.equal(durable.policyDigest,policyDigest);assert.equal(durable.materializedDigest,materializedDigest)}));
