...
export class NorthStarRunner{
 constructor({workspace,runtimeDir,taskStore,authorityGate=null}){this.workspace=workspace;this.runtimeDir=runtimeDir;this.tasks=taskStore;this.runs=new RunStore(runtimeDir);this.changesets=new ChangesetStore(runtimeDir,workspace);this.ws=new WorkspaceService(workspace);this.authorityGate=authorityGate}
 ...
 async validate(id,{approvalMode='workspace',commands=null}={}){let run=this.runs.get(id);if(!['CHANGES_APPLIED','REPAIR_REQUIRED','READY_FOR_DELIVERY'].includes(run.status))throw new Error('CHANGES_NOT_APPLIED');const selected=commands||run.validation?.commands||[];await this._authorize(id,'validation_execute',approvalMode,{commands:selected});run=this.runs.update(id,{status:'VALIDATING'},'VALIDATION_STARTED');this.tasks.syncRun(run);const result=await runAcceptance(this.workspace,selected,{mode:approvalMode});const status=result.passed?'READY_FOR_DELIVERY':'REPAIR_REQUIRED';run=this.runs.update(id,{status,validation:{...result,at:new Date().toISOString()},blocker:result.passed?null:'验收失败：需要 Repair'},'VALIDATION_FINISHED',{passed:result.passed,hardGate:result.hardGate});this.runs.addEvidence(id,{kind:'validation',status:result.status,hardGate:result.hardGate,results:result.results.map(x=>({command:x.command,code:x.code,timedOut:x.timedOut,durationMs:x.durationMs}))});run=this.runs.get(id);this.tasks.syncRun(run);return run}
 async repair(id,{approvalMode='workspace'}={}){
   ...
   this.runs.addEvidence(id,{kind:'repair-federated-generation',consensus:federation.consensus,winner:{provider:proposal.provider,role:proposal.role,evaluation:proposal.evaluation}});this.tasks.syncRun(this.runs.get(id));await this.apply(id,{approvalMode});return await this.validate(id,{approvalMode});
 }
