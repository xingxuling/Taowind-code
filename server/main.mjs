...
const runtimeDir=path.join(root,'runtime-data');const service=new WorkspaceService(workspace);const tasks=new TaskStore(runtimeDir);const browserOrgan=new BrowserKnowledgeOrgan(runtimeDir);let approvalMode='workspace';
...
const api=async(req,res,u)=>{
 if(req.method==='GET'&&u.pathname==='/api/health')return send(res,200,{ok:true,product:'Taowind Code',version:VERSION,workspace,approvalMode,providers:providerStatus(),authority:authorityGate.status(),northStar:'goal → autonomous mission → DWAC → RCL authority → changeset → validation → repair → closure audit → next cycle → evidence'});
 ...
 if(req.method==='GET'&&u.pathname==='/api/providers')return send(res,200,{...providerStatus(),authority:authorityGate.status(),browserOrgan:browserOrgan.status()});
...
const server=http.createServer(async(req,res)=>{try{const u=new URL(req.url,'http://localhost');if(u.pathname.startsWith('/api/'))return await api(req,res,u);if(staticFile(req,res))return;res.writeHead(404);res.end('Not found')}catch(e){const code=['RUN_NOT_FOUND','MISSION_NOT_FOUND'].includes(e.code)?404:e.code==='MISSION_BUSY'?409:['RCL_AUTHORITY_DENIED','APPROVAL_REQUIRED'].includes(e.code)?403:e.code==='RCL_RUNTIME_UNBOUND'?503:500;send(res,code,{error:e.code||e.message||'INTERNAL_ERROR',message:e.message,authorityReceipt:e.receipt||null})}});
