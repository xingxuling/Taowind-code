...
export function scoreProposal(p,{goal='',manifest=[]}={}){
  ...
  return {score:Number(score.toFixed(3)),coverage:Number(coverage.toFixed(3)),files,existingEdits,risky,hasValidation:!!p.validation_commands.length};
}
export function selectFederatedProposal(candidates,{goal='',manifest=[]}={}){
  ...
  const ranked=normalized.map(p=>({...p,evaluation:scoreProposal(p,{goal,manifest})})).sort((a,b)=>b.evaluation.score-a.evaluation.score||a.provider.localeCompare(b.provider)||a.role.localeCompare(b.role));
  const winner=ranked.find(x=>x.changes.length&&x.validation_commands.length)||ranked[0]||null;
  return {protocol:'taowind.federated-changeset-selection.v0.1',winner,ranked,consensus:{candidateCount:ranked.length,validCount:ranked.filter(x=>x.changes.length&&x.validation_commands.length).length,providers:[...new Set(ranked.map(x=>x.provider))]}};
}
