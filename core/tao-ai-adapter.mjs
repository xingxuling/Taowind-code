...
function closureSystemPrompt(role){return `You are a conservative North Star goal-closure auditor inside Taowind Code. ${CLOSURE_ROLES[role]} Return JSON only. Never treat plans, generated text, diffs, or model confidence as execution evidence. Schema: {"closed":boolean,"confidence":number,"reason":string,"gaps":[{"id":string,"problem":string,"severity":number,"evidence"?:string[]}],"next_goal":string,"recommended_mode":"WHOLE_ARTIFACT"|"DEEP_DEVELOPMENT"}. closed=true requires the supplied hard gates to pass and no material gap to remain.`}
