...
function rewrite(store,mission,evidence){const file=store.file(mission.id);const doc=JSON.parse(fs.readFileSync(file,'utf8'));doc.evidence=evidence;fs.writeFileSync(file,JSON.stringify(doc,null,2))}
...
test('durable mission evidence fails closed on forged identity or missing timestamp',()=>withStore(store=>{for(const evidence of [[{id:'forged',at:'2026-01-01T00:00:00.000Z'}],[{id:'mev-0123456789ab',at:''}],[null]]){const mission=store.create('x');rewrite(store,mission,evidence);assert.throws(()=>store.get(mission.id),error=>error instanceof Error&&error.message==='INVALID_MISSION_SHAPE')}}));
