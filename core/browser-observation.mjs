...
function boundedUniqueStrings(values,limit,code){const normalized=[...new Set((values||[]).map(String).filter(Boolean))];if(normalized.length>limit)throw Object.assign(new Error(code),{code,limit,observed:normalized.length});return normalized}
function normalizeCheck(raw={}){return {url:validateWebUrl(String(raw.url||'')),requiredSelectors:boundedUniqueStrings(raw.requiredSelectors||raw.selectors||[],MAX_REQUIRED_SELECTORS,'BROWSER_SELECTOR_BUDGET_EXCEEDED'),requiredText:boundedUniqueStrings(raw.requiredText||[],MAX_REQUIRED_TEXT,'BROWSER_TEXT_BUDGET_EXCEEDED'),titleIncludes:String(raw.titleIncludes||''),urlIncludes:String(raw.urlIncludes||''),forbidConsoleErrors:raw.forbidConsoleErrors!==false,forbidPageExceptions:raw.forbidPageExceptions!==false,forbidCriticalNetworkErrors:raw.forbidCriticalNetworkErrors!==false,screenshot:raw.screenshot!==false,settleMs:Math.max(100,Math.min(5000,Number(raw.settleMs||700)))}}
