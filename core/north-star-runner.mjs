...
import {captureValidationPostimage,compareValidationPostimage} from './validation-integrity.mjs';
...
export class NorthStarRunner{
 ...
 async validate(id,{approvalMode='workspace',commands=null,browserChecks=null}={}){let run=this.runs.get(id);if(!['CHANGES_APPLIED','REPAIR_REQUIRED','READY_FOR_DELIVERY'].includes(run.status))throw new Error('CHANGES_NOT_APPLIED');const selected=commands||run.validation?.commands||[];const checks=browserChecks||run.validation?.browserChecks||[];const browserRequired=run.validation?.browserRequired===true;const protectedPaths=[...new Set(run.changesets.flatMap(csid=>{try{return this.changesets.get(csid).changes.map(x=>x.path)}catch{return[]}}))];const postimageSnapshot=captureValidationPostimage(this.ws,protectedPaths);await this._authorize(id,'validation_execute',approvalMode,{commands:selected,browserChecks:checks,browserRequired});run=this.runs.update(id,{status:'VALIDATING'},'VALIDATION_STARTED');this.tasks.syncRun(run);let result=await runAcceptance(this.workspace,selected,{mode:approvalMode,browserObserver:this.browserObserver,browserChecks:checks,browserRequired});const postimageIntegrity=compareValidationPostimage(this.ws,postimageSnapshot);if(result.passed&&!postimageIntegrity.passed)result={...result,status:'FAILED',passed:false,hardGate:'VALIDATION_POSTIMAGE_DRIFT'};result={...result,postimageIntegrity};const status=result.passed?'READY_FOR_DELIVERY':'REPAIR_REQUIRED';run=this.runs.update(id,{status,validation:{...result,browserRequired,browserChecks:checks,at:new Date().toISOString()},blocker:result.passed?null:`验收失败：${result.hardGate}，需要 Repair`},'VALIDATION_FINISHED',{passed:result.passed,hardGate:result.hardGate,browserRequired,postimageIntegrity:postimageIntegrity.passed});this.runs.addEvidence(id,{kind:'validation',status:result.status,hardGate:result.hardGate,postimageIntegrity,results:result.results.map(x=>({command:x.command,code:x.code,timedOut:x.timedOut,durationMs:x.durationMs})),browser:(result.browser?.results||[]).map(x=>({id:x.id||null,url:x.url||x.check?.url||null,evidenceRoot:x.evidenceRoot||null,screenshot:x.screenshot||null,evaluation:x.evaluation||null,error:x.error||null}))});run=this.runs.get(id);this.tasks.syncRun(run);return run}
