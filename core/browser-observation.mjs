...
function normalizeSettleMs(value){const numeric=Number(value||700);if(!Number.isFinite(numeric))throw Object.assign(new Error('INVALID_BROWSER_SETTLE_MS'),{code:'INVALID_BROWSER_SETTLE_MS'});return Math.max(100,Math.min(5000,numeric))}
function browserStringField(raw,key){const value=raw[key];if(value===undefined)return '';if(typeof value!=='string')throw Object.assign(new Error('INVALID_BROWSER_CHECK_FIELD'),{code:'INVALID_BROWSER_CHECK_FIELD',field:key});return value}
function browserBooleanField(raw,key){const value=raw[key];if(value===undefined)return true;if(typeof value!=='boolean')throw Object.assign(new Error('INVALID_BROWSER_CHECK_FIELD'),{code:'INVALID_BROWSER_CHECK_FIELD',field:key});return value}
function normalizeCheck(raw={}){const selectorSource=raw.requiredSelectors!==undefined?raw.requiredSelectors:raw.selectors!==undefined?raw.selectors:[];const textSource=raw.requiredText!==undefined?raw.requiredText:[];return {url:validateWebUrl(String(raw.url||'')),requiredSelectors:boundedUniqueStrings(selectorSource,MAX_REQUIRED_SELECTORS,{containerCode:'INVALID_BROWSER_SELECTOR_LIST',itemCode:'INVALID_BROWSER_SELECTOR',budgetCode:'BROWSER_SELECTOR_BUDGET_EXCEEDED'}),requiredText:boundedUniqueStrings(textSource,MAX_REQUIRED_TEXT,{containerCode:'INVALID_BROWSER_REQUIRED_TEXT_LIST',itemCode:'INVALID_BROWSER_REQUIRED_TEXT',budgetCode:'BROWSER_TEXT_BUDGET_EXCEEDED'}),titleIncludes:browserStringField(raw,'titleIncludes'),urlIncludes:browserStringField(raw,'urlIncludes'),forbidConsoleErrors:browserBooleanField(raw,'forbidConsoleErrors'),forbidPageExceptions:browserBooleanField(raw,'forbidPageExceptions'),forbidCriticalNetworkErrors:browserBooleanField(raw,'forbidCriticalNetworkErrors'),screenshot:browserBooleanField(raw,'screenshot'),settleMs:normalizeSettleMs(raw.settleMs)}}
