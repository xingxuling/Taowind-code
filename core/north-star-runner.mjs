...
import {captureValidationPostimage,compareValidationPostimage,enforceValidationPostimage,checkDeliveryPostimage} from './validation-integrity.mjs';
...
export class NorthStarRunner{
 ...
 async delivery(id,{commit=false,message='',approvalMode='workspace'}={}){
   let run=this.runs.get(id);if(run.status!=='READY_FOR_DELIVERY')throw new Error('VALIDATION_REQUIRED');
   const deliveryIntegrity=checkDeliveryPostimage(this.ws,run.validation?.validatedPostimage);
   if(!deliveryIntegrity.passed){run=this.runs.update(id,{status:'REPAIR_REQUIRED',deliveryIntegrity,blocker:`交付失败：${deliveryIntegrity.hardGate}，验证后的代码现实已失效，需要重新验收或修复`},'DELIVERY_POSTIMAGE_REJECTED',{hardGate:deliveryIntegrity.hardGate,checked:deliveryIntegrity.checked,drift:deliveryIntegrity.drift.map(x=>({path:x.path,changed:x.changed}))});this.runs.addEvidence(id,{kind:'delivery-postimage-integrity',...deliveryIntegrity});run=this.runs.get(id);this.tasks.syncRun(run);return run}
   const changedPaths=[...new Set(run.changesets.flatMap(csid=>{try{return this.changesets.get(csid).changes.map(x=>x.path)}catch{return[]}}))];
   let delivery={...gitDeliveryPreview(this.workspace),changedPaths,postimageIntegrity:deliveryIntegrity};
   if(commit){
     await this._authorize(id,'git_commit',approvalMode,{changedPaths});
     const localCommit=gitLocalCommit(this.workspace,message||`Taowind Code: ${run.goal.slice(0,72)}`,changedPaths,run.validation?.validatedPostimage);
     delivery={...delivery,localCommit};
     if(localCommit?.indexIntegrity?.passed===false){
       const indexIntegrity=localCommit.indexIntegrity;
       run=this.runs.update(id,{status:'REPAIR_REQUIRED',delivery,deliveryIntegrity:indexIntegrity,blocker:`交付失败：${indexIntegrity.hardGate}，Git index（Git 暂存区）与已验证代码现实不一致，需要重新验收或修复`},'DELIVERY_GIT_INDEX_REJECTED',{hardGate:indexIntegrity.hardGate,checked:indexIntegrity.checked,drift:indexIntegrity.drift.map(x=>({path:x.path,changed:x.changed}))});
       this.runs.addEvidence(id,{kind:'delivery-git-index-integrity',...indexIntegrity});run=this.runs.get(id);this.tasks.syncRun(run);return run;
     }
   }
   run=this.runs.update(id,{status:commit&&delivery.localCommit?.ok?'DELIVERED_LOCAL':'READY_FOR_DELIVERY',delivery,deliveryIntegrity},'DELIVERY_PREVIEWED',{localCommitPerformed:!!delivery.localCommit?.ok,pushPerformed:false,prPerformed:false,changedPaths,postimageIntegrity:true});this.runs.addEvidence(id,{kind:'git-delivery',branch:delivery.branch,head:delivery.head,stat:delivery.stat,changedPaths,postimageIntegrity:true,localCommitPerformed:!!delivery.localCommit?.ok,pushPerformed:false,prPerformed:false});run=this.runs.get(id);this.tasks.syncRun(run);return run;
 }
}